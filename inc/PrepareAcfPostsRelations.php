...
class PrepareAcfPostsRelations 
{
	...
	private function __construct() {
        ...
        add_filter('cmk_posts_by_tabs_acf_posts_relations', function ($fields, $post, $request) {
            ...
            foreach($relational_field_keys as $key) {
                if (isset($fields[$key])) {
                    ...
                    $fields[$key] = array_map(function($post_id) use ($request) {
                    ...
                    $fields[$key] = array_filter($fields[$key], function($item) {
                        return !is_null($item);
                    });
